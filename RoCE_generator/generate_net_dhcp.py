...
def main():
    ...
    parser.add_argument("--starting-port", type=int, default=1, help="I use a port number for the metadata name e.g "network-port-1"  (default: 1)")
    ...
    for i in range(args.num_files):
        ...
        with open(f'network_attachment_{port}.yaml', 'w') as file:
            yaml.dump(network_attachment_yaml, file)

        with open(f'sriov_network_policy_{port}.yaml', 'w') as file:
            yaml.dump(sriov_network_policy_yaml, file)
